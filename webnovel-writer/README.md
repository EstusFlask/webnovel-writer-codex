...
长篇网文创作插件的源目录，当前同时包含 Claude Code 与 Codex 的插件入口。完整介绍、安装方式与文档导航见仓库根目录的 [README](../README.md)。
...
| 类型 | 数量 | 位置 |
|------|------|------|
| Skills（斜杠命令 / Codex 技能） | 9 | `skills/<name>/SKILL.md` |
...
| `/webnovel-doctor` | 只读体检项目文件、数据库、RAG、依赖与 Dashboard 产物 |

另有 `using-webnovel-writer` 作为 Codex 使用说明入口，不替代上述业务 Skill。
...
Codex 入口位于 `.codex-plugin/plugin.json`，同一套 `skills/` 与 Python runtime 会被复用。Codex 下运行命令前使用 `WEBNOVEL_PLUGIN_ROOT` 指向本插件根目录；Claude Code 仍兼容 `CLAUDE_PLUGIN_ROOT`。
...
python -X utf8 "<WEBNOVEL_PLUGIN_ROOT>/scripts/webnovel.py" --project-root "<PROJECT_ROOT>" preflight
