...
- Codex 或其他无 Claude `Agent` 工具的宿主必须进入兼容模式：按对应 agent 指令文件（`../../agents/context-agent.md`、`../../agents/reviewer.md`、`../../agents/data-agent.md`）的边界和 schema 在主流程执行，并在最终报告说明“未调用 subagent，使用兼容模式”；不得声称已经调用了不存在的 subagent
